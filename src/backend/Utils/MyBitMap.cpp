...
uint MyBitMap::getLeafData(int index) {
    return data[index];
}

void MyBitMap::setLeafData(int index, uint v) {
    data[index] = v;
}

int MyBitMap::setLeafBit(int index, uint k) {
    int pos, bit;
...
uint MyBitMap::childWord(int start, int bitNum, int i, int j) {
...
int MyBitMap::_setBit(uint* start, int index, uint k) {
    int pos, bit;
		getPos(index, pos, bit);
		uint umask = (1 << bit);
		uint mask = (~umask);
...
void MyBitMap::updateInner(int level, int offset, int index, int levelCap, uint k) {
...
void MyBitMap::setBit(int index, uint k) {
    //cout << data[0]<<endl;
...
MyBitMap::MyBitMap(int cap, uint k) {
    size = (cap >> BIAS);
    data = new uint[size];
    uint fill = 0;
    if (k == 1) {
...
MyBitMap::MyBitMap(int cap, uint* da) {
    data = da;
    size = (cap >> BIAS);
    init();
}

void MyBitMap::reLoad(uint* da) {
    data = da;
}